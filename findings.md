...
### HCM-015 Leave cancellation and auditable balance transactions — 2026-08-25

- Added `leave.leave_transactions` after the Leave request schema so the foreign key to `leave.leave_requests` is valid on a clean database. Mutable Leave balance counters remain projections; each reservation, approval, rejection release, pending cancellation, and approved cancellation records before/after used and pending values.
- Pending cancellation now runs through `IApprovalDecisionSideEffect.ApplyCancellationAsync`. It is requester/admin scoped by the approval controller, releases pending days atomically with the Leave request transition, and emits the Leave cancellation outbox event.
- The self-service cancellation permission is explicitly limited to `Leave` source approvals; it cannot cancel another approval type merely because the user is its requester. Administrative/approval cancellation permissions retain their existing governed boundary.
- Approved cancellation now runs through the existing `ILeaveActionContract` and a repository transaction that locks both request and balance. Direct cancellation of a pending request is refused with an explicit instruction to cancel the linked approval workflow; approved cancellation reverses used days and emits `CancelApproved`.
- OpenAPI sources were synchronized and Orval regenerated the Leave cancellation hook and optional approval cancellation reason. No new dependency or bounded-context database write was introduced.
- Runtime evidence against PostgreSQL 18 passed and was cleaned: pending cancellation returned 200 with `2 → 0` pending balance and `CancelPending`; approved cancellation returned 200 with `2 → 0` used balance and `CancelApproved`; stale row version returned 409; exact transaction, request, approval, outbox, balance, policy, type, identity-link, and manager-assignment cleanup returned zero/restored.
- Remaining Leave debt is escalation, schedules/holidays/overtime, accrual/adjustment/year-close policy, production identity claims, and full no-MSW Worker provenance. Cancellation balance release is no longer open.
- Post-correction validation: API build **0 warnings / 0 errors**, Architecture.Tests **197/197**, Vitest **19 files / 121 tests**, ESLint and Workforce TypeScript passed, workforce-web build passed, and full local Chromium Playwright **39/39 in 1.2m**. Existing non-failing React Aria/`act`, ECharts zero-size, and AG Grid trial-license warnings remain documented release debt.

### HCM-016 Cross-year Leave segmentation and balance isolation — 2026-08-25

- Removed the obsolete application-service rejection for cross-year requests after implementing domain-level inclusive `LeaveYearSegment` calculation.
- Leave submission, approval, pending cancellation, and approved cancellation now lock and update one configured balance per calendar year and record one auditable transaction per year with `balance_year`.
- The transaction migration adds `balance_year` and replaces the request/type uniqueness key with request/type/year so a cross-year lifecycle can retain one trail row per operation and year without weakening idempotency.
- Real PostgreSQL 18 runtime evidence passed with a temporary linked ESS identity and manager assignment: `2026-12-31` → `2027-01-02` reserved `1`/`2` days, approval used `1`/`2`, approved cancellation returned both used projections to zero, and a second pending cross-year request (`4`/`1`) was cancelled through Universal Approval and returned both pending projections to zero.
- Transaction evidence contained the expected per-year `ReservePending`, `Approve`, `CancelApproved`, and `CancelPending` rows. Exact temporary requests, approvals, outbox messages, balances, policy, leave type, identity link, and manager assignment cleanup returned `0|0|0|0`.
- Added `Phase3Tests.Leave_LeaveRequest_CrossYearRequest_SplitsIntoInclusiveYearSegments`; Architecture.Tests now pass **197/197**. The API rebuild passed **0 warnings / 0 errors** after removing the obsolete guard.
