...
import { PageHeader } from '@zainx/design-system/components/PageHeader/PageHeader';
import { FilterBar, type FilterItem } from '@zainx/design-system/components/FilterBar/FilterBar';
import {
  DensitySwitcher,
  type DensityType,
} from '@zainx/design-system/components/DensitySwitcher/DensitySwitcher';
import {
  ColumnChooser,
  type ColumnItem,
} from '@zainx/design-system/components/ColumnChooser/ColumnChooser';
import { BulkActionBar } from '@zainx/design-system/components/BulkActionBar/BulkActionBar';
...
import { ErrorState } from '@zainx/design-system/components/ErrorState/ErrorState';
...
export const AttendanceRecordsGrid: React.FC<AttendanceRecordsGridProps> = ({
  ...
  const getStatusBadge = (status: any, statusName?: string) => {
    ...
    if (s === 'Locked' || s === '3')
      return <Badge variant="neutral">{statusName || 'Locked'}</Badge>;
    if (s === 'Approved' || s === '2')
      return <Badge variant="success">{statusName || 'Approved'}</Badge>;
    if (s === 'Reviewed' || s === '1')
      return <Badge variant="primary">{statusName || 'Reviewed'}</Badge>;
  ...
  const columnDefs: ZainXColumnDef[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cellRenderer: (params: ICellRendererParams<AttendanceDayDto>) => (
          <span className="font-mono text-xs text-text-secondary">{params.value}</span>
        ),
      },
      {
        field: 'shiftName',
        ...
        cellRenderer: (params: ICellRendererParams<AttendanceDayDto>) => (
          <span className="text-xs text-text-secondary">{params.value || 'Standard Shift'}</span>
        ),
      },
      {
        field: 'firstInUtc',
  ...
  return (
    <div
      ...
      data-testid="attendance-records-grid"
    >
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3">
            ...
            <Button variant="outline" onClick={onRefresh} aria-label="Refresh attendance records">
              Refresh
            </Button>
          </div>
        }
      ...
      {isLoading ? (
        ...
        <div className="w-full overflow-hidden rounded-lg border border-border-secondary">
          <ZainXDataGrid columnDefs={columnDefs} rowData={filteredRecords} density={density} />
        </div>
      )}
    </div>
  );
